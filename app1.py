...
@app.route('/search/subcategories', methods=['GET'])
def search_subcategories():
    ...
    subcategories = SubCategory.query.filter(SubCategory.name.ilike(f"%{query}%")).all()
...
def search_products():
    ...
    response = [{
        ...
        'productProperties': [prop.to_dict() for prop in product.productProperties]
    } for product in products]

    return jsonify(response), 200


# Search applications by name or number
...
def search_applications():
    ...
    applications = Application.query.filter(
        db.or_(
            ...
            Application.number.ilike(f"%{query}%")
        )
    ).all()
